...
export const addChannelForUser = async (name: string): Promise<YouTubeChannelType> => {
  ...
  const [newChannel] = await db.insert(YouTubeChannels).values({ name, userId }).returning();
...
export const removeChannelForUser = async (id: string): Promise<void> => {
  ...
  await db.delete(YouTubeChannels).where(and(eq(YouTubeChannels.id, id), eq(YouTubeChannels.userId, userId)));
};